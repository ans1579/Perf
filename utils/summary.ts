...
function parseCaseName(
    name: string,
): { caseNo: string; caseName: string; suffix: string } | null {
    const idx = name.lastIndexOf(".");
    if (idx <= 0 || idx >= name.length - 1) return null;
    const raw = name.slice(0, idx).trim();
    const sep = raw.indexOf("::");
    if (sep < 0)
        return { caseNo: "", caseName: raw, suffix: name.slice(idx + 1) };
...
function compareCaseKey(a: string, b: string): number {
    ...
    if (ca.caseNo !== cb.caseNo)
        return ca.caseNo.localeCompare(cb.caseNo, "ko");
...
function readSummaryMeta(): SummaryMeta | null {
    ...
    try {
        const parsed = JSON.parse(
            fs.readFileSync(SUMMARY_META, "utf8"),
        ) as Partial<SummaryMeta>;
        if (
            !parsed ||
            typeof parsed.sourceStamp !== "string" ||
            typeof parsed.generatorStamp !== "string"
        )
            return null;
        return {
...
function buildCases(metrics: MetricLine[]): {
...
function rowSpread(
    ...
    const pct =
        min === 0
            ? null
            : Number((((max - min) / Math.abs(min)) * 100).toFixed(2));
    return { abs, pct };
}
        ...
        for (const t of targets) {
            ...
            const pct =
                baseVal === 0
                    ? null
                    : Number(
                          (((v - baseVal) / Math.abs(baseVal)) * 100).toFixed(
                              2,
                          ),
                      );
            lines.push([
...
function buildStatRows(metrics: MetricLine[], targets: string[]): StatRow[] {
    ...
    rows.sort((a, b) => {
        const c = compareCaseKey(
            makeCaseKey(a.caseNo, a.caseName),
            makeCaseKey(b.caseNo, b.caseName),
        );
        if (c !== 0) return c;
        const m = metricOrder(a.metric) - metricOrder(b.metric);
        if (m !== 0) return m;
        return (
            (targetOrder.get(a.target) ?? 999) -
            (targetOrder.get(b.target) ?? 999)
        );
    });
...
function writeSummaryStatsCsv(device: string, rows: StatRow[]) {
    ...
    const lines: string[][] = [
        ...
        [
            "해석 가이드",
            "p5/p95는 극단값 영향을 줄인 분포 범위입니다. n(반복횟수)이 많을수록 신뢰도가 높습니다.",
        ],
        [],
        [
            "단말",
    ...
    for (const row of rows) {
        const spread =
            row.max !== null && row.min !== null
                ? Number((row.max - row.min).toFixed(2))
                : null;
        const midSpread =
            row.p95 !== null && row.p5 !== null
                ? Number((row.p95 - row.p5).toFixed(2))
                : null;
        lines.push(
            [
                device,
...
function baselineState(
    ...
    if (ranked.length === 0)
        return { text: "데이터 없음", tone: "muted" as const };
    ...
    if (idx === ranked.length - 1)
        return { text: "개선 필요", tone: "bad" as const };
...
function toneFill(tone: "good" | "bad" | "mid" | "muted") {
    if (tone === "good")
        return {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: "FFE8F8EE" },
        } as const;
    if (tone === "bad")
        return {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: "FFFFECEC" },
        } as const;
    if (tone === "mid")
        return {
            type: "pattern",
...
async function _writeSummaryXlsx(
    device: string,
    rows: CompareRow[],
    targets: string[],
    statRows: StatRow[],
    ...
    const baseline = targets[0] ?? "";
    const baselineTargetIndex = baseline
        ? targets.findIndex((t) => t === baseline)
        : -1;
    const metrics: MetricKey[] = [
        "e2eMs",
        "memoryDeltaMB",
    ...
    const ws1 = wb.addWorksheet("한눈요약", {
    ...
    for (const metric of metrics) {
        ...
        if (best) {
            ...
            if (idx >= 0)
                ws1.getCell(row.number, 3 + idx).fill = toneFill("good");
        }
        if (worst) {
            const idx = targets.findIndex((t) => t === worst.target);
            if (idx >= 0)
                ws1.getCell(row.number, 3 + idx).fill = toneFill("bad");
        }
    }
    ...
    for (const rowData of rows) {
        ...
        if (best) {
            const idx = targets.findIndex((t) => t === best.target);
            if (idx >= 0)
                ws2.getCell(row.number, 5 + idx).fill = toneFill("good");
        }
        if (worst) {
            const idx = targets.findIndex((t) => t === worst.target);
            if (idx >= 0)
                ws2.getCell(row.number, 5 + idx).fill = toneFill("bad");
        }
    }
    autoFitColumns(ws2, 10, 46);

    const ws3 = wb.addWorksheet("반복통계", {
        views: [{ state: "frozen", ySplit: 1 }],
    });
    const statHeaders = [
        "번호",
...
function summarizeRepeatReliability(statRows: StatRow[]): {
    label: string;
    detail: string;
    tone: "good" | "mid" | "bad" | "muted";
} {
    const counts = statRows
        .map((r) => r.count)
        .filter((n) => Number.isFinite(n) && n > 0);
    if (counts.length === 0) {
        return {
            label: "데이터 없음",
            detail: "반복 통계가 아직 없습니다.",
            tone: "muted",
        };
    }

    const min = Math.min(...counts);
    const avgCount = Number(
...
function buildInsight(
    ...
    if (!baseline) {
        ...
        return {
            title,
            tone: "muted" as const,
            text: `최저값은 ${best.target} (${best.value})입니다.`,
        };
    }
    ...
    const diffPct =
        best.value === 0
            ...
            : Number(
                  (
                      ((base.value - best.value) / Math.abs(best.value)) *
                      100
                  ).toFixed(2),
              );
    if (rank === total) {
...
function makeRangeFromNumbers(nums: number[]): MetricRange {
    if (nums.length === 0)
        return { min: 0, max: 0, paddedMin: 0, paddedMax: 1 };
    const min = Math.min(...nums);
    const max = Math.max(...nums);
    const span = max - min;
...
function buildRunTrendData(
    metrics: MetricLine[],
    targets: string[],
): Partial<Record<MetricKey, RunTrendMetricData>> {
    ...
    for (const metric of metricKeys) {
        const candidates = [...byMetricCase.entries()].filter(([k]) =>
            k.startsWith(`${metric}${CASE_KEY_SEP}`),
        );
        ...
        for (const key of selectedKeys) {
            const m = meta.get(key);
            if (!m) continue;
            const sorted = [...(raw.get(key) ?? [])]
                .sort((a, b) => a.ts - b.ts)
                .map((p) => round(p.value, 2) ?? p.value);
        ...
        const maxRun = Math.max(
            1,
            ...Object.values(seriesByTarget).map((s) => s.length),
        );
...
function makeMetricRanges(
    rows: CompareRow[],
    targets: string[],
): Record<MetricKey, MetricRange> {
    ...
    for (const key of keys) {
        ...
        const span = max - min;
        const pad =
            span === 0 ? Math.max(Math.abs(min) * 0.08, 1) : span * 0.08;
        out[key] = {
            min: round(min, 2) ?? min,
            max: round(max, 2) ?? max,
            paddedMin: min - pad,
...
function panel(
    ...
    const body = filtered
        .map((r) => {
            ...
            const rowRange =
                rowMin === null || rowMax === null ? null : rowMax - rowMin;
            const rowSpreadPct =
            ...
            const cols = targets
                .map((t) => {
                    ...
                    if (v !== null && rowMin !== null) {
                        if (v === rowMin) deltaText = "기준<br />&nbsp;";
                        else
                            deltaText = `+${(v - rowMin).toFixed(2)}${percentDelta(rowMin, v)}`;
                    } else {
                        deltaText = "N/A<br />&nbsp;";
                    }
                    return `
      ...
      <div class="bar-row">
        <div class="bar-head">
          ...
          <div class="bar-meta">
            min ${format(rowMin === null ? null : round(rowMin, 2))}
...
function runTrendCard(
    ...
    const x = (runIndex: number) =>
        data.maxRun <= 1
            ? pad.left + plotW / 2
            : pad.left + (runIndex / (data.maxRun - 1)) * plotW;
    const y = (value: number) =>
        pad.top + (1 - (value - data.range.paddedMin) / span) * plotH;
    ...
    const xLabels = tickIndexes
        .map(
            (i) =>
                `<text x="${x(i)}" y="${height - 10}" text-anchor="middle" class="axis-text">${i + 1}</text>`,
        )
        .join("");
    ...
    const caseTitle = data.caseNo
        ? `${data.caseNo}. ${data.caseName}`
        : data.caseName;
    return `
  <section class="trend-card">
    <div class="trend-head">
...
function writeSummaryHtml(
    ...
    const legend = targetList
        .map(
            (t) =>
                `<span class="legend-item"><i class="legend-dot" style="background:${c[t]}"></i>${escapeHtml(t)}</span>`,
        )
        .join("");
    const tableRows =
        rows.length === 0
            ...
            : rows
                  .map((r) => {
                      const nums = targetList
                          ...
                          .filter((v): v is number => v !== null);
                      const rowMin =
                          nums.length === 0 ? null : Math.min(...nums);
                      const rowMax =
                          nums.length === 0 ? null : Math.max(...nums);
                      const vals = targetList
                          .map((t) => {
                              const v = r.values[t];
                              const best =
                                  v !== null && rowMin !== null && v === rowMin;
                              const worst =
                                  v !== null &&
                                  rowMax !== null &&
                                  rowMax !== rowMin &&
    ...
    const statRowsHtml = statRows
        .map((r) => {
            const spread =
                r.max !== null && r.min !== null
                    ? Number((r.max - r.min).toFixed(2))
                    : null;
            const midSpread =
                r.p95 !== null && r.p5 !== null
                    ? Number((r.p95 - r.p5).toFixed(2))
                    : null;
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .trend-tooltip { position: fixed; z-index: 999; pointer-events: none; display: none; max-width: 320px; padding: 6px 8px; border-radius: 8px; border: 1px solid #c9d7ef; background: rgba(18, 31, 54, 0.94); color: #f8fbff; font-size: 11px; line-height: 1.35; box-shadow: 0 6px 22px rgba(15, 23, 42, 0.28); white-space: nowrap; }
    ...
    .v-delta { font-size: 10px; color: #64748b; min-height: 28px; line-height: 1.35; text-align: center; }
...
<body>
  ...
  <script>
    (() => {
      ...
      const charts = document.querySelectorAll('.trend-card svg');
      ...
      const place = (event) => {
        const pad = 12;
        tooltip.style.left = String(event.clientX + pad) + 'px';
        tooltip.style.top = String(event.clientY + pad) + 'px';
      };
      ...
      dots.forEach((dot) => {
        ...
        dot.addEventListener('mouseout', hide);
      });
...
async function writeSummaryPdf() {
    ...
    const generateSummaryPdf =
        mod.generateSummaryPdf ?? mod.default?.generateSummaryPdf;
...
export async function generateSummaryArtifacts(
    ...
    const force =
        options.force === true || isTruthyEnv(process.env.PERF_SUMMARY_FORCE);
    ...
    if (!force && hasRequiredSummaryOutputs()) {
        ...
        if (
            prev?.sourceStamp === sourceStamp &&
            prev?.generatorStamp === GENERATOR_STAMP
        )
            return;
    }
    ...
    try {
        ...
        console.warn(
            "[summary] summary_stats.csv 생성 실패:",
            (e as Error).message,
        );
    }
